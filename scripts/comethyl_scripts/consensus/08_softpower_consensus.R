...
#     3) Selects a single shared consensus power only when every dataset has
#        both R2 >= cutoff AND a negative scale-free regression slope
...
choose_consensus_power <- function(all_summary_list, cutoff = 0.8) {
  ...
  power_summary <- combined %>%
    ...
    dplyr::summarise(
      n_datasets      = dplyr::n(),
      n_meet_cutoff   = sum(Valid_Topology, na.rm = TRUE),
  ...
  # Never silently select power 1 when no dataset combination passes. In the
  # old code, all powers tied at zero passing datasets and the smallest tie won.
  # That produced a syntactically valid but scientifically invalid power file.
  list(power   = NA_integer_,
       reason  = paste0("no common power had R2 >= ", cutoff,
                        " with negative slope in every dataset"),
       summary = power_summary)
}
...
for (ds in dataset_inputs) {
  ...
  slope_col  <- get_col(c("slope"))
  mean_col   <- get_col(c("mean.k.","mean.k"))
  median_col <- get_col(c("median.k.","median.k"))
  max_col    <- get_col(c("max.k.","max.k"))

  if (any(is.na(c(power_col, fit_col, slope_col, mean_col, median_col, max_col))))
    stop("Could not identify expected columns in SFT table for dataset: ", ds_label)

  summary_df <- tibble::tibble(
    Dataset             = ds_label,
    Power               = sft_table[[power_col]],
    # WGCNA's SFT.R.sq is already the fit statistic. Do not use abs() because
    # that can make a signed/negative diagnostic value look acceptable.
    SFT_R2              = sft_table[[fit_col]],
    Slope               = sft_table[[slope_col]],
    Valid_Topology      = sft_table[[fit_col]] >= opt$scale_free_cutoff &
                          sft_table[[slope_col]] < 0,
...
if (is.na(chosen_power)) {
  warning("No valid shared consensus power was selected. Script 09 will stop ",
          "if this chosen_power.txt is supplied; review the benchmark/QC first.")
} else {
...
make_combined_softpower_plot(
  ...
  chosen_power = if (is.na(chosen_power)) NULL else chosen_power
)
