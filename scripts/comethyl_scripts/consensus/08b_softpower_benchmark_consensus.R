...
#     present in all provided datasets, so the same regions are
#     scored for every dataset within a seed.
...
choose_consensus_power <- function(summary_df, cutoff = 0.8) {
  power_summary <- summary_df %>%
    ...
    dplyr::summarise(
      ...
      n_meet_cutoff   = dplyr::n_distinct(Dataset[Valid_Topology]),
...
common_regions <- Reduce(intersect, lapply(meth_list, rownames))
