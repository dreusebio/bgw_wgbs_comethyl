...
parse_combo <- function(path) {
  ...
  rel <- sub(paste0("^", benchmark_root), "", path, fixed = TRUE)
  rel <- sub("^/+", "", rel)  # drop any leading slash left after stripping the literal prefix
...
rank_table <- all_summaries %>%
  ...
  dplyr::summarise(
    ...
    n_meet_cutoff = dplyr::n_distinct(Dataset[Mean_SFT_R2 >= opt$scale_free_cutoff]),
